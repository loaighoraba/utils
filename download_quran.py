...
class Quran:
    ...
    def download(self):
        for sura_number in range(1, len(Quran.SURA_NAMES) + 1):
            sura = Sura(sura_number, Quran.SURA_NAMES[sura_number - 1])
            sura.download(self.url, self.directory)
            print(f"{sura.number} downloaded")
...
url = "https://download.quran.islamway.net/quran3/965/212/128/"

downloader = Quran(url, directory)
downloader.download()